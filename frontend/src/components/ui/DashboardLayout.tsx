    ...
    <div className="min-h-screen bg-paper flex">
      <aside className="w-64 bg-paper-raised border-r border-slate-light flex flex-col">
        <div className="p-6 border-b border-slate-light">
          ...
          <p className="text-xs text-slate font-mono-data mt-1">{user.role}</p>
        </div>
        ...
        <nav className="flex-1 p-4 flex flex-col gap-1">
          {navItems.map((item) => (
              ...
              key={item.href}
              href={item.href}
              className="px-3 py-2 rounded-md text-sm text-ink-soft hover:bg-paper hover:text-ink transition-colors"
            >
              {item.label}
            </a>
          ))}
